...
| 21h | Intl SMS codes | Source inconsistencies: the API guide's consolidated table glosses `011` as "Undelivered" while the code list gives it as "Teleservice not provisioned" (`UNDELIV` is the class, not the meaning); `001`/`033` both read "Unknown subscriber" and `027`/`029` both "Absent subscriber" | `011` documented as teleservice not provisioned; duplicates documented as equivalent with a note. (The "Value (hex)" heading is settled: the full platform list runs `900128`, `900129`, `900130`, so the codes are decimal) | Confirm the duplicate pairs really are equivalent |
...
| 23b | Platform error codes | **`900xxx` codes are channel-scoped and collide — on five codes, not one.** The full SMS platform list (Sept 2026) shows `900118`–`900122` all mean different things on SMS than on WhatsApp/RCS: encoding limit vs template category blocked, sender ID restricted vs template category changed, SMPP account disabled vs monthly frequency limit, TUC disabled vs daily frequency limit, TUC validity expired vs blacklisted. `900115` (frequency limit) is the only shared code that agrees | Warnings on all three pages now name the `900118`–`900122` range | **Is the collision intentional? A single cross-channel platform code range would be far easier to consume** |
...
| 29 | Intl SMS codes | The full SMS platform list (Sept 2026, 31 codes) was added to the international page. Three things it leaves open: (a) it gives **`900116` Credit Limit Exceeds**, while the API guide gives **`900210` Credit limit breached** (`UNDELIV`) — two codes for what reads as one condition; (b) it carries no `ERRORDESP`, so `REJECTD` is confirmed for only five of the codes (`900088`, `900101`, `900111`, `900112`, `900118`); (c) it skips `900117`, which is Timeout on WhatsApp and RCS | Both credit codes documented and handled alike; page says "most" arrive as `REJECTD` and names the two known exceptions | **Owner: Shivangi.** Which credit code does an international receipt actually carry, and do they differ? Confirm the `ERRORDESP` for each code — especially `900114` expired message and `900125` time window, which could plausibly be `EXPIRED` |
| 29b | India SMS codes | The India `970`–`995` band is the same platform list word for word, in the same order, under three-digit codes (`970`↔`900088`, `971`–`987`↔`900100`–`900116`, `988`–`995`↔`900118`–`900125`). But the platform list has five codes past that point with **no three-digit counterpart**: `900126` unauthorised source address, `900127` variable length exceeded, `900128` invalid/inactive sender ID, `900129` blacklisted country, `900130` duplicate message drop | India page now shows the six-digit code beside each `970`–`995` row | **Owner: Srishti.** What does an India receipt carry when one of those five fires — a DLT-band code (`204`, `211`), a generic one, or the six-digit code? If duplicates are dropped on India traffic, customers need to know |
