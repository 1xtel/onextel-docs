...
| 21g | Intl SMS codes | The international `ERRORCODE` set is described as remappable on the platform, so the default table is not a contract | Documented with a `<Warning>` telling integrators to confirm the mapping applied to their own account | **Where is remapping configured, and is it per-account, per-route, or per-operator?** Docs should say who to ask |
...
| 21i | Intl SMS | `clientsmsid` and `custref1`–`custref5` are two separate reference mechanisms. The observed DLR carrying custrefs had no `CLIENTMSID`, and vice versa | Documented as two mechanisms, either usable, echoed back only if sent | **Confirm whether both can be sent on the same message and both returned** |
| 21j | Intl SMS codes | The **Retry?** guidance on the international error-code page is inferred from the standard meaning of each GSM/SMPP condition, not taken from a OneXtel source | Marked explicitly as guidance, not a platform guarantee | Have the routing team confirm or correct the retryable set |
