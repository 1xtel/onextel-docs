...
| 2 | SMS send | Send path: PDF `/send_sms` vs Postman `/oapi/send_sms` | **Resolved (Sept 2026) — both are live and both are correct.** Probed with a rejected key: each returns `401 API_Invalid_Authorisation` but with a differently shaped envelope, so they are separate handlers rather than an alias. `http://` answers `301` to HTTPS. Both paths and the HTTPS point are now documented | **Open: exactly which features `/oapi/send_sms` lacks.** OneXtel reports `/send_sms` is the fuller one; the field list on the page has only been verified against `/oapi` |
...
| P19 | CleverTap | The campaign personalization screenshot (the approach confirmed with Karthikeyan, Sept 2026) carries a campaign-level **`sender_id`** pair set to `The Body Shop_1`. The documented campaign key-value table does not list `sender_id`, and the provider step lists only `message_type` and `template_id` as `$$` body parameters | Not documented — screenshot kept as supplied | **Is `sender_id` required, optional, or specific to accounts with several sender profiles? If it is used, does the provider also need `sender_id: $$sender_id`?** |
