...
| 20e | SMS error codes | Code `995` arrives mojibake-encoded (`Platform_Time window Ã¢â‚¬â€œ discarded`) | **Confirmed (Sept 2026):** the platform's own code sheet carries the same mojibake, so the reading "Time window — discarded" is right and the encoding is wrong at source | Fix the encoding in the platform code list |
...
| 23b | Platform error codes | **`900xxx` codes are channel-scoped and collide — on five codes, not one.** The India SMS platform list (Sept 2026) shows `900118`–`900122` all mean different things on SMS than on WhatsApp/RCS: encoding limit vs template category blocked, sender ID restricted vs template category changed, SMPP account disabled vs monthly frequency limit, TUC disabled vs daily frequency limit, TUC validity expired vs blacklisted. `900118` is also an encoding limit on international SMS. `900115` (frequency limit) is the only shared code that agrees | WhatsApp and RCS warnings name the `900118`–`900122` range and link the India SMS page | **Is the collision intentional? A single cross-channel platform code range would be far easier to consume** |
...
| 23e | Error codes | The unified list supplies no HTTP status for any code | **Explained (Sept 2026):** Meta publishes HTTP statuses only for the Marketing Messages API, not for Cloud API codes, so omitting the column is correct rather than a gap | Confirm the `500` our older source gives for Meta code `3` — see 26 |
...
| 29b | India SMS codes | **India SMS has two forms of every platform code.** The domestic hub records six-digit codes (`hub2/src/erls/dbupdate.erl` writes `900088` on throttle, `900111` on no balance, `900113` on connection error), while the customer-facing SMPP list gives the same checks as `970`–`995`, word for word and in order (`970`↔`900088`, `971`–`987`↔`900100`–`900116`, `988`–`995`↔`900118`–`900125`). Five newer checks — `900126` unauthorised source address, `900127` variable length exceeded, `900128` invalid/inactive sender ID, `900129` blacklisted country, `900130` duplicate message drop — have **no three-digit form**. Separately, an older list (`aura-config/custom_scripts/pinelabs/aura_error_codes.csv`) gives `900115` as "Duplicate Check API"; the current list has it as SMS frequency limit, with duplicates moved to `900130` | India page lists both forms side by side, says the platform records both, and lists the five as six-digit only. It does not say which form a DLR carries | **Owner: Srishti.** Which form does the customer's DLR callback carry — three-digit or six-digit? When one of the five newer checks fires, what does the customer receive? Was `900115` repurposed from duplicate check to frequency limit? |
| 2b | SMS send | `custRef1`–`custRef5` accept either a plain string or a nested object (`CustomReference1`, `CustomReference2`). **The nested form is SMS-only** — WhatsApp and RCS take plain values | Documented on the send page with both forms and a note scoping the object form to SMS | Confirm whether the nested object is echoed back whole on the DLR, and whether it counts against the five slots |
| 2c | SMS send | Content verified only on `/oapi/send_sms` was **removed** from the `/send_sms` page rather than carried over: the `type` rejection matrix (`TRANS`/`Promo`/`TXN`, lowercase and omitted → `700 invalid type`), the Rejected response example, and the claim that per-message rejections still return HTTP 200. `/send_sms` validates less — it accepted recipient `12345` — so none of these can be assumed. A `/status` lookup of that `/send_sms` message ID returned `No Records found` | The page now says only: use `SI`/`OTP`/`SE`; `200` means accepted for processing, not delivered and not a valid number; confirm via the DLR | **Re-verify on `/send_sms`: which `type` values are rejected and with what status; what a per-message rejection looks like; whether `/status` finds `/send_sms` message IDs; whether a nested `custRef` is echoed on the DLR.** Needs test sends — ask first, because `/send_sms` does not reject undeliverable recipients |
| P19 | CleverTap | Campaign-level `sender_id` | **Answered (Sept 2026): optional, only for accounts with more than one sender profile.** Documented in Part C, the personalization reference, and the parameter reference. The provider step still lists only `message_type` and `template_id` as `$$` body parameters, on the assumption that a campaign `sender_id` pair is forwarded the same way placeholder pairs are | Confirm a campaign `sender_id` works without also adding `sender_id: $$sender_id` to the provider |
| P20 | MoEngage | CleverTap template creation is now documented as optional, because the campaign `template_id` pair selects the OneXtel template. MoEngage is documented differently: its campaign form makes **Select template** mandatory, and template name and ID must match | MoEngage page keeps template creation as a required Part B | **Confirm MoEngage genuinely needs the template, rather than the same optional treatment as CleverTap** |
...
| 26 | WhatsApp error codes | Meta code `3` (API Method) is mapped to HTTP **500** while every other permission error is 403 | Reproduced as given | **Meta publishes no HTTP status for Cloud API code 3, so this cannot be settled from public docs — confirm internally or drop the value** |
...
| P21 | CleverTap, MoEngage, WebEngage | **The V2 SMS routes were not live in production on 22 Sep 2026.** `ct.onex-aura.com/sms/send` and `/pinelab/sms/send`, `mo.onexaura.com/moengage/sms` and `/moengage/sms/dlr`, and `we.onexaura.com/webengage/{sms,whatsapp,rcs}/dlr` (plus `/dsn`, `/tracking/events`) all answered 404; the older routes on the same hosts answered normally. The SMS tabs and the WebEngage delivery-report setup describe these routes, so the PR should merge only once they answer. |
| P22 | MoEngage | WhatsApp and RCS sends report success for any HTTP 2xx without reading OneXtel's per-message result, so a rejection OneXtel returns inside HTTP 200 shows in MoEngage as sent. A OneXtel 401 on WhatsApp becomes `7004`, not `7000`. Not documented as behaviour — fix in the connector (the SMS route already reads the per-message status). |
| P23 | MoEngage | **RCS→SMS fallback receipts cannot be matched in MoEngage.** The RCS send reply returns only `callback_data`, never a OneXtel message ID, while the SMS receipt is keyed on `msgID` and posted to the SMS provider's URL. The RCS tab still documents the SMS fallback callback. How should MoEngage attribute a fallback SMS? |
| P24 | CleverTap | Two connector issues: a missing required SMS body field is sent to OneXtel as the literal text `undefined` rather than rejected; and the repo does not pin the SMS path — without `ONEXTEL_SMS_SEND_PATH=/send_sms` on the server it falls back to `/oapi/send_sms`. The docs state `/send_sms` per the directive, so confirm production sets it. |
| P25 | WebEngage | The repo's `production.env` carries no `WE_*_URL` / `WE_*_TOKEN` settings for SMS reports, and the code's default path is `/tracking/events`, while testing used `/tracking/privatessp-events`. Production must set all six before SMS reports flow. Separately, the WhatsApp table in `errorCodeMap.ts` is shadowed by `whatsapp-codes.ts` (27 of 64 values differ) though the README cites it — the docs publish the effective table. |
| P26 | WebEngage SMS | For sends made straight to OneXtel's `api.onexaura.com/api/we/`, is the `messageId` on a `WebEngage`-format receipt the same as WebEngage's own messageId? The docs register the callback with Callback Format `WebEngage`, as in the Sept setup sheet; verify on the first live receipt. |
