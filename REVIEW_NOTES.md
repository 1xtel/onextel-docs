...
| 2d | WebEngage SMS connector | `Webengage-Native` `src/sms/send.ts:273` (`interpret`, omni branch) reads `b.smsList`. On `/send_sms` the key is `smslist`, so an **accepted** message is reported to WebEngage as failed, status `9988` "Unexpected Onextel response". The code default for `ONEXTEL_IN_SEND_URL` is still `/oapi/send_sms`, but the box-only `production.env` can override it | Docs unaffected | **Check the deployed `ONEXTEL_IN_SEND_URL`. If it points at `/send_sms`, read `b.smsList ?? b.smslist` before the SMS relay carries live traffic** |
