...
| 21d | MoEngage RCS | The MoEngage setup guide and the OneXtel sender-profile screen both show **OTP** as a valid RCS message type (`Name_OTP`, `SenderID_OTP`, and an Otp tag on a live sender profile), but the product decision recorded earlier was that RCS supports only Promotional and Transactional via plugins | Docs still state Promotional and Transactional only | **Reconcile: is OTP available for RCS via plugins or not?** |
...
| 21f | Connector guides | MoEngage callback screenshots contain **live API keys and an ngrok dev URL**; the CleverTap guide references `https://production_url` as a placeholder | Those images were not published; the CleverTap page uses `{connectorBaseUrl}` | **Supply the production connector URL, and re-capture the callback screenshots without keys** |
