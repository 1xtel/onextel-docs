...
| 23 | RCS / WhatsApp error codes | Source tables give operator codes but never state which field carries them | **Answered for WhatsApp (Meta docs):** `error.code` on an API response, and `entry.changes.value.errors[]` — or `...messages.errors[]` — on a webhook | Still open for the RCS operator codes: which field carries a Jio or Vi code |
